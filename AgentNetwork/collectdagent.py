...
class Collect:
    ...
    def packString(self, type_code, string):
        return struct.pack("!HH", type_code, 2 + 2 + len(string) + 1)
...
class CollectdAgent:
    ...
    def reportCPU(self):
        ...
        for cpu in cpus:
            ...
            data = b"".join([data, self.C.pack(TYPE_TYPE_INSTANCE, "idle")])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "percent")])
            data = b"".join([data, self.C.packValues([VALUE_GAUGE, int(cpu.idle)])])
            data = b"".join([data, self.C.pack(TYPE_TYPE_INSTANCE, "user")])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "percent")])
            data = b"".join([data, self.C.packValues([VALUE_GAUGE, int(cpu.user)])])
            data = b"".join([data, self.C.pack(TYPE_TYPE_INSTANCE, "interrupt")])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "percent")])
            data = b"".join([data, self.C.packValues([VALUE_GAUGE, int(cpu.interrupt)])])
            data = b"".join([data, self.C.pack(TYPE_TYPE_INSTANCE, "system")])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "percent")])
            data = b"".join([data, self.C.packValues([VALUE_GAUGE, int(cpu.system)])])
            i = i + 1
            if len(buffer) + len(data) > 1400:
                self.sendMessage(buffer)
                buffer = b"".join([header, data])
            else:
    ...
    def reportNetwork(self):
        ...
        for nic in net:
            ...
            data = b"".join([data, self.C.pack(TYPE_TYPE, "if_octets")])
            data = b"".join([data, self.C.packValues([VALUE_COUNTER, n.bytes_recv], [VALUE_COUNTER, n.bytes_sent])])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "if_packets")])
            data = b"".join([data, self.C.packValues([VALUE_COUNTER, n.packets_recv], [VALUE_COUNTER, n.packets_sent])])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "if_dropped")])
            data = b"".join([data, self.C.packValues([VALUE_COUNTER, n.dropin], [VALUE_COUNTER, n.dropout])])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "if_errors")])
            data = b"".join([data, self.C.packValues([VALUE_COUNTER, n.errin], [VALUE_COUNTER, n.errout])])
            if len(buffer) + len(data) > 1400:
                self.sendMessage(buffer)
                buffer = b"".join([header, data])
            else:
        ...
        for p in partitions:
            if 'fixed' in p.opts:
                try:
                    ...
                    self.logger.logit("Data trasfered to IDS Server - packets, memory, disk")
        self.sendMessage(buffer)
    ...
    def reportDiskIO(self):
        ...
        for diskname in io:
            ...
            data = b"".join([data, self.C.pack(TYPE_TYPE, "disk_octets")])
            data = b"".join([data, self.C.packValues([VALUE_COUNTER, d.read_bytes], [VALUE_COUNTER, d.write_bytes])])
            data = b"".join([data, self.C.pack(TYPE_TYPE, "disk_ops")])
            data = b"".join([data, self.C.packValues([VALUE_COUNTER, d.read_count], [VALUE_COUNTER, d.write_count])])
            if len(buffer) + len(data) > 1400:
                self.sendMessage(buffer)
                buffer = b"".join([header, data])
            else:
    ...
    def reportMemory(self):
        ...
        buffer = b"".join([header, self.C.pack(TYPE_TYPE, "memory")])
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "used")])
        buffer = b"".join([buffer, self.C.packValues([VALUE_GAUGE, p.used])])
        #
        # buffers and cached do not seem to be present on windows...
        # ... but just in case they show up in future...
        try:
            i = p.buffers
            buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "buffers")])
            buffer = b"".join([buffer, self.C.packValues([VALUE_GAUGE, i])])
        except:
            i = 0
        try:
            i = p.cached
            buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "cached")])
            buffer = b"".join([buffer, self.C.packValues([VALUE_GAUGE, i])])
        except:
            i = 1
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "free")])
        buffer = b"".join([buffer, self.C.packValues([VALUE_GAUGE, p.available])])
        s = psutil.swap_memory()
        buffer = b"".join([buffer, self.C.pack(TYPE_PLUGIN, "swap")])
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE, "swap")])
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "used")])
        buffer = b"".join([buffer, self.C.packValues([VALUE_GAUGE, s.used])])
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "free")])
        buffer = b"".join([buffer, self.C.packValues([VALUE_GAUGE, s.free])])
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE, "swap_io")])
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "in")])
        buffer = b"".join([buffer, self.C.packValues([VALUE_DERIVE, s.sin])])
        buffer = b"".join([buffer, self.C.pack(TYPE_TYPE_INSTANCE, "out")])
        buffer = b"".join([buffer, self.C.packValues([VALUE_DERIVE, s.sout])])
        #print(buffer)
        self.sendMessage(buffer)
...
class Agent:        
    ...
    def __init__(self):
        ...
        self.agent.buildSocket("127.0.0.1")
